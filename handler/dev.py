...
class DevHandler(BaseHandler):
    def post(self, url):
        ...
        if url.startswith('git'):
            try:
                post_data = json.loads(self.request.body.decode(encoding='utf8'))
            except BaseException:
                return
            print(post_data.get("ref", '').split('/')[-1])
            if self.request.headers.get('X-Github-Event', '-1') == 'push' \
                    and post_data.get("ref", '').split('/')[-1] == os.environ['GIT_BRANCH']:
                ...
                print('Bad args,', self.request.headers.get('X-Github-Event', '-1'), post_data.get("ref", ''))
